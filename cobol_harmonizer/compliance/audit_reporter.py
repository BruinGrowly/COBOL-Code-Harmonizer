...
class ComplianceReporter:
            ...
            if assessment.risk_level == RiskLevel.CRITICAL:
                ...
                violation = ComplianceViolation(
                    ...
                    evidence={
                        ...
                        "compliance_tags": [
                            tag.value for tag in assessment.compliance_tags
                        ],
    ...
    def _assess_framework_status(
        ...
        for framework, relevant_tags in framework_tag_mapping.items():
            ...
            critical_count = sum(
                1 for a in relevant if a.risk_level == RiskLevel.CRITICAL
            )
    ...
    def _extract_high_risk_changes(
        self, assessments: List[RiskAssessment]
    ) -> List[Dict]:
    ...
    def export_markdown(self, report: AuditReport, output_path: str) -> None:
        ...
        md.append(
            f"- **Total Procedures Analyzed:** {report.total_procedures_analyzed}"
        )
        ...
        if report.framework_status:
            ...
            for framework, status in report.framework_status.items():
                icon = (
                    "✅"
                    if status == "Compliant"
                    else ("⚠️" if "Review" in status else "❌")
                )
...
"""
    ...
    def _generate_html_framework_status(self, report: AuditReport) -> str:
        ...
        for framework, status in report.framework_status.items():
            ...
            icon = (
                "✅"
                if css_class == "compliant"
                else ("⚠️" if css_class == "needs-review" else "❌")
            )
