...
class DisharmonyCalculator:
    ...
    def calculate(self, intent: LJPWCoords, execution: LJPWCoords) -> float:
        """
        ...
        distance = math.sqrt(
            (l_i - l_e) ** 2 + (j_i - j_e) ** 2 + (p_i - p_e) ** 2 + (w_i - w_e) ** 2
        )
    ...
    def explain_disharmony(
        self, intent: LJPWCoords, execution: LJPWCoords, disharmony_score: float
    ) -> str:
        """
        ...
        explanations = {
            DisharmonyLevel.HARMONIOUS: "Function name accurately reflects implementation. "
            "Intent and execution are well-aligned.",
            DisharmonyLevel.MINOR_DRIFT: f"Minor semantic drift detected. Function name suggests {from_dim} "
            f"operations, but implementation leans toward {to_dim}. "
            "Consider reviewing for clarity.",
            DisharmonyLevel.CONCERNING: f"Concerning contradiction detected. Function name strongly suggests "
            f"{from_dim} domain, but implementation operates in {to_dim} domain. "
            "This may confuse developers and cause bugs.",
            DisharmonyLevel.SIGNIFICANT: f"Significant semantic disharmony! Function name promises {from_dim} "
            f"behavior but delivers {to_dim} behavior. This is likely to cause "
            "production issues. Rename or refactor immediately.",
            DisharmonyLevel.CRITICAL: f"CRITICAL SEMANTIC BUG! Function name explicitly says it does "
        ...
        return explanations.get(level, "Unable to classify disharmony level.")

    def calculate_detailed_analysis(
        self, procedure_name: str, intent: LJPWCoords, execution: LJPWCoords
    ) -> Dict:
        """
